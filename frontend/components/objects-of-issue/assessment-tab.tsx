...
import {
  OBJECTS_CRITERION_STATE_LABELS,
  type ObjectsCriterionState,
} from '@/lib/objects-of-issue/assessment';
...
const COUNT_STATE_KEYS: Array<{ key: keyof typeof EMPTY_COUNTS; state: ObjectsCriterionState }> = [
  ...
  { key: 'pendingSupportingSource', state: 'pending_supporting_source' },
...
function stateTone(state: string): string {
  switch (state as ObjectsCriterionState) {
    case 'reconciled':
...
function stateLabel(state: string): string {
  return (
    OBJECTS_CRITERION_STATE_LABELS[state as ObjectsCriterionState] ??
    state.replaceAll('_', ' ')
  );
}

export function ObjectsOfIssueAssessmentTab() {
  const { assessment, derivedError, refreshDerived } = useObjectsOfIssue();

  if (!assessment) {
    return (
      <div className="space-y-3">
        <p className="text-sm text-muted-foreground" role="status">
          {derivedError ?? 'Loading Objects Assessment…'}
        </p>
        {derivedError ? (
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <Metric label="Objects recorded" value={String(metrics.objects)} />
        ...
        <Metric
          ...
          value={
            metrics.netFreshIssueProceeds ? formatMoney(metrics.netFreshIssueProceeds) : EM_DASH
          }
        />
        <Metric
          label="Total estimated cost of objects"
          value={
            metrics.totalEstimatedObjectsCost
              ? formatMoney(metrics.totalEstimatedObjectsCost)
              : EM_DASH
          }
        />
      </div>
