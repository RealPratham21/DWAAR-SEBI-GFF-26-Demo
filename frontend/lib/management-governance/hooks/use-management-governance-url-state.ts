...
import {
  MANAGEMENT_GOVERNANCE_INFORMATION_SECTIONS,
...
const DEFAULT_TAB: ManagementGovernanceTabId = 'information';
const DEFAULT_SECTION: ManagementGovernanceSectionId =
  'board-structure-and-ipo-governance-readiness';

function isTab(value: string | null | undefined): value is ManagementGovernanceTabId {
  return MANAGEMENT_GOVERNANCE_TABS.some((tab) => tab.id === value);
}

function isSection(value: string | null | undefined): value is ManagementGovernanceSectionId {
  return MANAGEMENT_GOVERNANCE_INFORMATION_SECTIONS.some((section) => section.id === value);
}

export function useManagementGovernanceUrlState(initial?: { tab?: string; section?: string }) {
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();
  const skipNextSyncRef = useRef(false);

  const [activeTab, setActiveTabState] = useState<ManagementGovernanceTabId>(() => {
    if (isTab(initial?.tab)) return initial.tab;
    const fromUrl = searchParams.get('tab');
    if (isTab(fromUrl)) return fromUrl;
    return DEFAULT_TAB;
  });
  const [activeSection, setActiveSectionState] = useState<ManagementGovernanceSectionId>(() => {
  ...
  const writeUrl = useCallback(
    (next: { tab?: ManagementGovernanceTabId; section?: ManagementGovernanceSectionId }) => {
  ...
  const setActiveSection = useCallback(
    (section: ManagementGovernanceSectionId) => {
