"""Objects of the Issue workspace model — one row per user."""
...
class ObjectsIssueWorkspace(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    __tablename__ = "objects_issue_workspaces"
    __table_args__ = (
        Index("ix_objects_issue_workspaces_user_id", "user_id", unique=True),
    )
    ...
    user: Mapped["User"] = relationship(back_populates="objects_issue_workspace")
