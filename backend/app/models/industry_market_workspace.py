"""Industry & Market workspace model — one row per user."""
...
class IndustryMarketWorkspace(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    __tablename__ = "industry_market_workspaces"
    __table_args__ = (
        Index("ix_industry_market_workspaces_user_id", "user_id", unique=True),
    )
    ...
    user: Mapped["User"] = relationship(back_populates="industry_market_workspace")
