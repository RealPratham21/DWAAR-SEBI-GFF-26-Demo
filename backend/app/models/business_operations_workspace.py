"""Business & Operations workspace model — one row per user."""
...
class BusinessOperationsWorkspace(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    __tablename__ = "business_operations_workspaces"
    __table_args__ = (
        Index("ix_business_operations_workspaces_user_id", "user_id", unique=True),
    )
    ...
    user: Mapped["User"] = relationship(back_populates="business_operations_workspace")
