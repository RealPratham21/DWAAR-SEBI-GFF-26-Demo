"""Financials & KPIs workspace model — one row per user."""
...
class FinancialsKpisWorkspace(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    __tablename__ = "financials_kpis_workspaces"
    __table_args__ = (
        Index("ix_financials_kpis_workspaces_user_id", "user_id", unique=True),
    )
    ...
    user: Mapped["User"] = relationship(back_populates="financials_kpis_workspace")
