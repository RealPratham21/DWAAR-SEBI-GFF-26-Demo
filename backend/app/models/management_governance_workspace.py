"""Management & Governance workspace model — one row per user."""
...
class ManagementGovernanceWorkspace(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    __tablename__ = "management_governance_workspaces"
    __table_args__ = (
        Index("ix_management_governance_workspaces_user_id", "user_id", unique=True),
    )
    ...
    user: Mapped["User"] = relationship(back_populates="management_governance_workspace")
