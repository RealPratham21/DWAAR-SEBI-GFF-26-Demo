"""Capital & Ownership workspace model — one row per user."""
...
class CapitalOwnershipWorkspace(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    __tablename__ = "capital_ownership_workspaces"
    __table_args__ = (
        Index("ix_capital_ownership_workspaces_user_id", "user_id", unique=True),
    )
    ...
    user: Mapped["User"] = relationship(back_populates="capital_ownership_workspace")
