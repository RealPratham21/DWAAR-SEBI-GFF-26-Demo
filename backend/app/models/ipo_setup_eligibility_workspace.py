"""IPO Setup & Eligibility workspace model — one row per user."""
...
class IpoSetupEligibilityWorkspace(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    __tablename__ = "ipo_setup_eligibility_workspaces"
    __table_args__ = (
        Index("ix_ipo_setup_eligibility_workspaces_user_id", "user_id", unique=True),
    )
    ...
    user: Mapped["User"] = relationship(back_populates="ipo_setup_eligibility_workspace")
