"""Objects of the Issue HTTP routes."""
...
from app.modules.objects_issue.service import (
    get_assessment,
    get_overview,
    get_workspace,
...
router = APIRouter(
    prefix="/workstreams/objects-issue",
    tags=["objects-issue"],
)
...
@router.get("/workspace", response_model=ObjectsIssueWorkspaceResponse)
...
@router.get("/objects-assessment", response_model=ObjectsAssessmentResponse)
def get_objects_assessment_endpoint(
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
) -> ObjectsAssessmentResponse:
