"""Group Entities & Related Parties HTTP routes."""
...
router = APIRouter(
    prefix="/workstreams/group-entities-related-parties",
...
@router.get("/workspace", response_model=GroupEntitiesWorkspaceResponse)
...
@router.get("/group-rpt-assessment", response_model=GroupAssessmentResponse)
def get_group_rpt_assessment_endpoint(
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
) -> GroupAssessmentResponse:
