...
from app.modules.drhp.export.styles import DRAFT_FOOTER_NOTICE, DRAFT_HEADER, EXPORT_TYPOGRAPHY


def _set_run_font(run, *, size_pt: float, bold: bool = False) -> None:  # noqa: ANN001
...
def _add_paragraph(document: Document, text: str, *, style_name: str | None = None, bold: bool = False) -> None:
    ...
    if not paragraph.runs:
        ...
        run.text = text
    _set_run_font(run, size_pt=EXPORT_TYPOGRAPHY.body_size_pt, bold=bold)
...
class DRHPDocxRenderer:
    ...
    def _configure_header_footer(self, section, document: DRHPExportDocument) -> None:  # noqa: ANN001
        ...
        header.text = DRAFT_HEADER
        header.alignment = WD_ALIGN_PARAGRAPH.CENTER
        if header.runs:
            _set_run_font(header.runs[0], size_pt=EXPORT_TYPOGRAPHY.header_size_pt, bold=True)
        if document.issuer_name:
            ...
            if issuer_para.runs:
                _set_run_font(issuer_para.runs[0], size_pt=EXPORT_TYPOGRAPHY.header_size_pt - 1)
        ...
        if footer.runs:
            _set_run_font(footer.runs[0], size_pt=EXPORT_TYPOGRAPHY.footer_size_pt)

        page_field = section.footer.add_paragraph()
        page_field.alignment = WD_ALIGN_PARAGRAPH.CENTER
        run = page_field.add_run("Page ")
        _set_run_font(run, size_pt=EXPORT_TYPOGRAPHY.footer_size_pt)
    ...
    def _build_toc(self, doc: Document, document: DRHPExportDocument) -> None:
        ...
        _set_run_font(run, size_pt=EXPORT_TYPOGRAPHY.toc_title_size_pt, bold=True)
        if document.partial_label:
            _add_paragraph(doc, f"({document.partial_label})", bold=False)
        for order, chapter_title in document.table_of_contents:
            para = doc.add_paragraph(f"{order}. {chapter_title}")
            para.paragraph_format.left_indent = Mm(6)
            if para.runs:
                _set_run_font(para.runs[0], size_pt=EXPORT_TYPOGRAPHY.body_size_pt)
        notice = doc.add_paragraph(DRAFT_FOOTER_NOTICE)
        if notice.runs:
            _set_run_font(notice.runs[0], size_pt=EXPORT_TYPOGRAPHY.legal_notice_size_pt)

    def _build_chapter(self, doc: Document, chapter: ExportChapter) -> None:
        heading = doc.add_heading(chapter.title, level=1)
        if heading.runs:
            _set_run_font(heading.runs[0], size_pt=EXPORT_TYPOGRAPHY.chapter_title_size_pt, bold=True)
    ...
    def _build_section(self, doc: Document, section: DrhpSectionAST) -> None:
        if section.heading:
            ...
            if heading.runs:
                _set_run_font(heading.runs[0], size_pt=EXPORT_TYPOGRAPHY.section_heading_size_pt, bold=True)
    ...
    def _build_block(self, doc: Document, block: DrhpBlockAST) -> None:
        ...
        if block.kind == "heading":
            ...
            if heading.runs:
                _set_run_font(
                    heading.runs[0],
                    size_pt=EXPORT_TYPOGRAPHY.subsection_heading_size_pt,
                    bold=True,
                )
            return
        ...
        if block.kind == "legal_notice":
            para = doc.add_paragraph(str(content.get("text") or ""))
            if para.runs:
                _set_run_font(para.runs[0], size_pt=EXPORT_TYPOGRAPHY.legal_notice_size_pt)
            return

        if block.kind in {"bullet_list", "numbered_list"}:
            style = "List Number" if block.kind == "numbered_list" else "List Bullet"
    ...
    def _build_table(self, doc: Document, content: dict[str, Any]) -> None:
        ...
        if caption:
            ...
            if cap.runs:
                _set_run_font(cap.runs[0], size_pt=EXPORT_TYPOGRAPHY.subsection_heading_size_pt, bold=True)
        ...
        if headers:
            for col_index, header in enumerate(headers):
                ...
                if cell.paragraphs and cell.paragraphs[0].runs:
                    _set_run_font(cell.paragraphs[0].runs[0], size_pt=EXPORT_TYPOGRAPHY.table_size_pt, bold=True)
            row_index = 1
        ...
        for row in rows:
            ...
            for col_index, value in enumerate(row[:col_count]):
                ...
                cell.text = str(value)
                if cell.paragraphs and cell.paragraphs[0].runs:
                    _set_run_font(cell.paragraphs[0].runs[0], size_pt=EXPORT_TYPOGRAPHY.table_size_pt)
            row_index += 1
