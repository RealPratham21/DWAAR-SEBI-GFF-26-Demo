"""Business & Operations HTTP routes."""
...
from app.modules.business_operations.service import (
    get_assessment,
    get_overview,
    get_workspace,
...
router = APIRouter(
    prefix="/workstreams/business-operations",
...
@router.get("/workspace", response_model=BusinessOperationsWorkspaceResponse)
...
@router.get("/business-assessment", response_model=BusinessAssessmentResponse)
def get_business_assessment_endpoint(
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
) -> BusinessAssessmentResponse:
