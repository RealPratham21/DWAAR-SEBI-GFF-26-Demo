"""Management & Governance HTTP routes."""
...
from app.modules.management_governance.service import (
    get_assessment,
    get_overview,
    get_workspace,
...
router = APIRouter(
    prefix="/workstreams/management-governance",
...
@router.get("/workspace", response_model=ManagementGovernanceWorkspaceResponse)
...
@router.get("/governance-assessment", response_model=GovernanceAssessmentResponse)
def get_governance_assessment_endpoint(
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
) -> GovernanceAssessmentResponse:
